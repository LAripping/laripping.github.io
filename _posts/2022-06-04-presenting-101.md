...
{% include note.html content="At the moment this step's  advice only applies to fully remote presentation setups, where video is recorded and uploaded ahead of the event" %}
...
* Removing whole segments of bad takes or silence, if we're pressed for time
  ...
  * [NCH] Hit "L" 
    ...
    {% include note.html content='The tools allows you to split "only video" or "only audio" beyond the default setting of splitting both' %}
...
* Background noise reduction  (laptop fan)
  ...
  {% include tip.html content='If you use the same laptop to edit-out the noise from the recording laptop, connect some noise-cancelling headphones, to keep track of where the noise comes from.<br/><br/>Also, <strong>don\'t</strong> noise-reduct more than once maybe twice, or it will end up sounding too "electric" / "metallic"' %} 
